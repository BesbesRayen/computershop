...
public class ProductAdapter extends RecyclerView.Adapter<ProductAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (product != null) {
            ...
            // UPDATED: Load product image - Support for Base64 and URL images
            if (product.getImageUrl() != null && !product.getImageUrl().isEmpty()) {
                if (isBase64Image(product.getImageUrl())) {
                    // Base64 image - decode and display
                    loadBase64Image(holder.productImage, product.getImageUrl());
                } else {
                    // URL image - use Glide for Firebase URLs
    ...
    private boolean isBase64Image(String imageString) {
        return imageString != null &&
                (imageString.length() > 1000 || // Base64 strings are typically long
                        !imageString.startsWith("http")); // Not a URL
    }
        ...
        FirebaseManager.getCartItems(currentUserId, new FirebaseManager.FirebaseCallback() {
            ...
            public void onSuccess(Object result) {
                ...
                for (CartItem item : cartItems) {
                    if (item.getIdArt() != null && item.getIdArt().equals(product.getIdArt())) {
                        existingItem = item;
                        break;
                    }
                }
